...
<%
	...
	List<Graduateinfo> infoList 	= (List<Graduateinfo>) request.getAttribute("infoList");
	List<Gradeall> scoreList 		= (List<Gradeall>) request.getAttribute("scoreList");
	int canjiadabian 	= 0;
	int tongguodabian 	= 0;
	int weidabian 		= 0;
 %>
									...
									<%
										if(!Verify.isEmpty(sefList)) {
											for(int i = 0; i < sefList.size(); i ++) {
												...
												Float opentopicScore 	= (float) 0.0;
												if((!Verify.isEmpty(infoList))) {
													for(int j = 0; j < infoList.size(); j ++) {
														Graduateinfo opt = infoList.get(j);
														if(opt.getStuId().equals(se.getStuId())) {
															//参加答辩
															isHaveOpentopic = true;
															if(!Verify.isEmpty(scoreList)) {
																for(int k = 0; k < scoreList.size(); k ++) {
																	Gradeall opts = scoreList.get(k);
																	if(opts.getStuId().equals(se.getStuId())) {
																		opentopicScore = opts.getGaGrade();
																		break;
																	}
																}
