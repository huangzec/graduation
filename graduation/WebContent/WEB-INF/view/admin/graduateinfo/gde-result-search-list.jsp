...
%>
<%
	List<Gradeall> list 		= (List<Gradeall>) request.getAttribute("gdescoreList");
	...
	List<Graduateinfo> gdeinfoList 	= (List<Graduateinfo>) request.getAttribute("gdeinfoList");
	List<Gradeall> gdescoreList 	= (List<Gradeall>) request.getAttribute("gdescoreList");
	int canjiadabian 	= 0;
	int tongguodabian 	= 0;
	int weidabian 		= 0;
 %>
<form id="pagerForm" method="post" action="<%=basePath %>admin/graduateinfo/totalresult.do">
...
<div class="pageHeader">
	<form method="post" action="<%=basePath %>admin/graduateinfo/totalresult.do" id="score-search" class="pageForm required-validate" onsubmit="return navTabSearch(this);">
	<div class="searchBar">
		<div>
			请选择查询条件：
				...
				<%
					if(!Verify.isEmpty(stuList)) {
						...
						if((!Verify.isEmpty(gdeinfoList))) {
							for(int j = 0; j < gdeinfoList.size(); j ++) {
								Graduateinfo opt = gdeinfoList.get(j);
								if(opt.getStuId().equals(stu.getStuId())) {
									//参加答辩
									isHaveOpentopic = true;
									if(!Verify.isEmpty(gdescoreList)) {
										for(int k = 0; k < gdescoreList.size(); k ++) {
											Gradeall opts = gdescoreList.get(k);
											if(opts.getStuId().equals(stu.getStuId())) {
												opentopicScore = (double) opts.getGaGrade();
												break;
											}
										}
