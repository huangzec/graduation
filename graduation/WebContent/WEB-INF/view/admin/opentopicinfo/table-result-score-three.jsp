...
%>
<%
	List<Gradethree> scoreList 	= (List<Gradethree>) request.getAttribute("scoreList");
 ...
 %>
<form id="pagerForm" method="post" action="<%=basePath %>admin/opentopicinfo/tablethreescore.do">
...
<div class="pageHeader">
	<form method="post" action="<%=basePath %>admin/opentopicinfo/tablethreescore.do" id="score-search-list" class="pageForm required-validate" onsubmit="return navTabSearch(this);">
	<div class="searchBar">
		<div>
			请选择查询条件：
						...
						for(int i = 0; i < stuList.size(); i ++) {
							...
							if(!Verify.isEmpty(scoreList)) {
								for(int j = 0; j < scoreList.size(); j ++) {
									Gradethree score = scoreList.get(j);
									if(score.getStuId().equals(student.getStuId())) {
										one 	= score.getGtrOne();
										two 	= score.getGtrTwo();
										three 	= score.getGtrThree();
										four 	= score.getGtrFour();
										five 	= score.getGtrFive();
										six 	= score.getGtrSix();
										all 	= score.getGtrAll();
										break;
									}
